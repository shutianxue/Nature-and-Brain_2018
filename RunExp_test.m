function RunExp_test(subjectID, version)
...
round = 2; % no. of exp rounds (stimulus+VAS)
           % default: 3
...
% time
time_limit = 5;  % max time for answering one question
                 % default: 30
safe_time = 0.3; % short pause to separate presses
                 % default: .3
                 
rest_PR = 1;     % default: 1
rest_time1 = 1%*60; % compulsory rest time (in minutes) between rounds 
                   % default: 2(mins)*60
rest_time2 = 1%*60; % compulsory rest time (in minutes) between scales
                   % default: 25(mins)*60
                   
% 2. image
num_img = 2; % no. of images retrived from one scale, location: pwd/images/subfolders
              % default: 15 pictures; 
time_img = 0.5; % how long each image lasts, in secs
              % default: 8 secs
...
%{
%% VAS + image + gap Loop (4 times, each for one scale)
